...
static const char* REFERENCE_RIL_VERSION = "RIL_RK_DATA_V10.0"
#ifndef RIL_RELEASE
    " [debug version]"
#endif
#if !ENABLE_DNS_CHECK
...
#define LOG_NDEBUG 0
#define LOG_TAG "RIL_RK_DATA_V10.0"
#include <utils/Log.h>
...
GET_OEM_Strings get_oem_strings(const char * data)
{
    if (strStartsWith(data, "ICCID"))
	    return OEM_STRINGS_ICCID;
    return OEM_STRINGS_ERROR;
}

static void requestOemHookStringsIccid(void *data, size_t datalen, RIL_Token t)
{
    char *response[1];
    memset(response, 0, sizeof(response));
    response[0] = "89860114838032731058";
...
/*	case RIL_REQUEST_VOICE_RADIO_TECH:
	...
	case RIL_REQUEST_OEM_HOOK_STRINGS: {
	    int i;
	    const char *strings = ((const char**)data)[0];
	    RLOGD("> '%s'", strings);
	    switch(get_oem_strings(strings)) {
		    case OEM_STRINGS_ICCID:
			    requestOemHookStringsIccid(data, datalen, t);
			    break;
		    default:
			    RLOGE("Cannot parse this string");
